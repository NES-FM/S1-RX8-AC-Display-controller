 ...
 *  Modified 2023 NES-FM
 *  for Arduino Mega 2560
 *
 ******************************************************/

#include <SPI.h>
// #include <Time.h> // Seems to be unused
#include <Wire.h>
// #include "DS3231.h"
#include "RTClib.h"
...
void iconCalculateMode(byte _modeSet) // Mode 1 = feet, 2 = feet\demist, 3 = face, 4 = face\feet
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (!bitRead(acAmpRxData[4], 2) && bitRead(acAmpRxData[4], 6)) // feet/demist has dependancies on addition demist related bytes
        {
            iconArray[0][12] += 0x02;
            iconArray[0][12] += 0x04;
        }
...
void iconDefaultDisplayValues()
{
    ...
    if (acAmpOn)
    {
        ...
        bitRead(acAmpRxData[4], 3) ? iconArray[0][7] += 0x09 : iconArray[0][7] += 0x05; // airSource
        if (acAmpAmbient)
        {
            iconArray[0][6] += 0x02;                                               // AMB
            (acAmpCentigrade) ? iconArray[0][6] += 0x10 : iconArray[0][6] += 0x08; // C or F
        }
...
void sevenSegmentCalculate(byte _column, byte _value)
{
    ...
    {
    case 0:
        sevenSegmentArray[0][0] += _column;
        sevenSegmentArray[0][1] += _column;
        sevenSegmentArray[0][2] += _column;
        sevenSegmentArray[0][4] += _column;
        sevenSegmentArray[0][5] += _column;
        sevenSegmentArray[0][6] += _column;
        break;
    case 1:
        sevenSegmentArray[0][2] += _column;
        sevenSegmentArray[0][5] += _column;
...
void longButtonAction(byte _longPushButton)
{
    ...
    if (!confMode) // we can set control options on long button press
    {
        ...
        case 7:
            break;
        case 8:
            acAmpAmbient = !acAmpAmbient ? true : false;
            acAmpDataOut[2] = 0xA0;
            acAmpDataOut[4] = 0xDC;
            break;
        }
...
void confMenu()
{
    ...
    {
    case 1:
        populateFixedLcdDisplay("Temp C or F");
        if (shortPushButton == 1)
        {
            acAmpCentigrade = !acAmpCentigrade;
            acAmpCentigrade ? iconArray[0][6] += 0x10 : iconArray[0][6] += 0x08;
        }
        break;
    case 2:
        populateFixedLcdDisplay("24/12 Hour");
        if (clock24Hour)
        {
        ...
        if (shortPushButton == 1)
            clock24Hour = (!clock24Hour) ? true : false;
        break;
    case 3:
        menuIndexMax = 3;
        updateClockTime();
