...
void display::init() {
    ...
    SPI.beginTransaction(
        SPISettings( 1000000, MSBFIRST,
                     SPI_MODE3 ) ); // I couldn't see a defined max speed in the NJU6623 datasheet, 10Meg is quite an
                                    // increase on the original 9600, so need to monitor incase this is not stable.
...
/*****************************************************************
  ...
  // int icon9A = [10]; Auto-M,            TP,       TA,                       RDM,          Full Stop between DDRAM #10
& #11
  // int icon9B = [11]; not used,          fan7,   min/sec prime marks, R/H vertical divider, Full Stop between DDRAM
#11 & #12
...
void display::setDefaultText() { writeToCharDisp( defaultText ); }
...
// Parameters; the HEX value for the bit mapped to column (0x10 = 8C Hh, 0x08 = 8C hH, 0x01 = 8D Tmp, 0x04 = 8D tMp,
// 0x02 = 8C tmP)
...
void display::lcdDisplayCgramToDdram( byte _CgramSource,
                                      byte _DdramDestination ) // Copies a CGRam char into the DDram field
{
    PORTB &= ~( 1 << PB0 );
